...
def takeimageNG(j):
    ...
    while True:
        success, frame = cap.read()
        cv.imshow("Sample NG Image", frame)
        key = cv.waitKey(1)
        if key == ord('s'):
            cv.imwrite(j + "/master-image-NG.jpg", frame)
            flag_ok = True
            break
        if key == 27:
            break
    if flag_ok == True:
        ...
        while i<50:
            ...
            img_cropped = gray_img[top_left[1]:top_left[1] + h,top_left[0]:top_left[0] + w ]#img[top : bottom , left : right]
            strnum = str(i)
            cv.imwrite(j+"/image_data_NG/img"+strnum+".jpg", img_cropped)
            print("Image data captured "+strnum)
            i = i + 1
    cap.release()
    cv.destroyAllWindows()
...
def learning():
    ...
    X = []
    Y = []
    lokasi = masterselect.get() + "/image_data"
    lokasiNG = masterselect.get() + "/image_data_NG"
    for base, dirs, files in os.walk(lokasi):
        ...
        for file in files:
            ...
            print(np.shape(data))
            X.append(data)
            Y.append(0)#0 for OK image
            cnt += 1
    for base, dirs, files in os.walk(lokasiNG):
        print("Searching in :", base)
        cnt = 0
        for file in files:
            if cnt == 50:
                break
            cntstr = str(cnt)
            img = cv.imread(lokasiNG+"/img"+cntstr+".jpg", 0)
            img_resize = cv.resize(img,(img_size, img_size))
            data = np.asarray(img_resize)
            data = data.reshape([-1])
            print(np.shape(data))
            X.append(data)
            Y.append(1)#1 for NG image
            cnt += 1
    print(np.shape(X))
    print(np.shape(Y))
    ...
    with open(lokasi+"/model.pickle", mode='wb') as f:
        pickle.dump(clf, f)
    with open(lokasi+"/model.pickle", mode='rb') as f:
        clf = pickle.load(f)
...
def predict():
    ...
    print(predict)
    clf = GradientBoostingClassifier()
    with open(masterselect.get() + "/image_data/model.pickle", mode='rb') as f:
        clf = pickle.load(f)
    X = []
    cap = cv.VideoCapture(0)
    success, frame = cap.read()
    img_gray = cv.cvtColor(frame, cv.COLOR_BGR2GRAY)
    template = cv.imread(masterselect.get() + "/master-template.jpg", 0)
    w,h = template.shape[::-1]
    ...
    if preds == 0:
        ...
        cv.putText(frame, "GOOD", org=top_left, fontFace=cv.FONT_HERSHEY_TRIPLEX, fontScale=0.5, color=(0, 255, 0), thickness=1)
        print("OK")
    else:
        cv.rectangle(frame, top_left, bottom_right, (0,0,255), thickness=2)
        cv.putText(frame, "NOT GOOD", org=top_left, fontFace=cv.FONT_HERSHEY_TRIPLEX, fontScale=0.5, color=(0, 0, 255), thickness=1)
        print("NG")
...
part_05 = tkinter.OptionMenu(app, masterselect, *masterlist, command=select)
